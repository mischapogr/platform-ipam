...
| [Operator UI authentication](GUI_AUTHENTICATION.md) | Audit of the NetBox UI exposure; basic, Entra ID and LDAP design. Not implemented |
...
| [NetBox AWS plugin](NETBOX_AWS_PLUGIN.md) | Desk evaluation of plugins that model AWS accounts, VPCs and subnets in NetBox. Nothing installed |
| [Onboarding import](ONBOARDING_IMPORT.md) | Importing existing accounts, networks and ranges from CSV, Excel or a pasted table. Not implemented |
...
| [ADR 0016](decisions/0016-SOURCE_AWARE_REFRESH_AND_REMOVAL_OF_IMPORTED_OCCUPANCY.md) | Imported occupancy names its contributors in an unowned NetBox field; a re-import writes only when the contributor set changes; a removal is one named prefix, explicit, and refused without complete coverage for every contributing account and region (accepted as a design 2026-09-22; built through packages M9b1-M9b4 — `onboard apply --refresh` and `onboard remove` — against the development NetBox only; stage and production have no creator for the custom fields yet) |
| [ADR 0017](decisions/0017-PERSISTING_ONLY_WHAT_A_LEDGER_TRANSACTION_CHANGED.md) | The ledger port and its whole-state closures stay; a write persists only the difference between the state it loaded and the state it was handed back, audit history is no longer re-offered on every write, and the inventory projection skips a `PATCH` that would change nothing (accepted as a design and an intermediate step 2026-09-22; not built — the row-lock model stays the target) |
...
Still unverified: live AWS, a Kubernetes rollout, provider distribution and
installation (development runs use a CLI development override and never
exercise registry resolution, checksums, or the lock file), and the pinned
NetBox compatibility matrix beyond the single version the local stack pins.
