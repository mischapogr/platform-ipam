...
| [ADR 0017](decisions/0017-PERSISTING_ONLY_WHAT_A_LEDGER_TRANSACTION_CHANGED.md) | The ledger port and whole-state closures stay. Audit writes (M4e), the remaining table diff and on-demand audit reads (M4f), and M4g's projection `PATCH` skip are built and locally tested, including a synthetic 1,000-allocation timing run. Representative customer performance remains unverified; per-aggregate row locks remain the target. |
